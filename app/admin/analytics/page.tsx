...
export default function AdminAnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-slate-900">Sales Analytics</h1>
        <p className="text-slate-600 mt-1">View sales trends and top products</p>
      </div>
              ...
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase">Units Sold</th>
                  ...
                  {analytics.topProducts && analytics.topProducts.length > 0 ? (
                    analytics.topProducts.map((product: any, idx: number) => (
                      <tr key={product._id || idx} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 text-sm text-slate-700">{product.totalSold}</td>
                        <td className="px-6 py-4 text-sm font-semibold text-slate-900">
              ...
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase">Orders</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase">Revenue</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {analytics.salesData && analytics.salesData.length > 0 ? (
                    analytics.salesData.map((sale: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 text-sm text-slate-700">{sale.orderCount}</td>
                        <td className="px-6 py-4 text-sm font-semibold text-slate-900">
